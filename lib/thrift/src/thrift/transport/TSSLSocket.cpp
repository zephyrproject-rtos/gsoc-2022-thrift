...
void TSSLSocket::openSecConnection(struct addrinfo* res) {
  ...
  int ret = setsockopt(socket_, SOL_TLS, TLS_SEC_TAG_LIST, sec_tag_list, sizeof(sec_tag_list));
  if (ret != 0) {
    throw TTransportException(TTransportException::NOT_OPEN, "set TLS_SEC_TAG_LIST failed");
  }

  ret = setsockopt(socket_, SOL_TLS, TLS_PEER_VERIFY, &(ctx_->verifyMode),
                   sizeof(ctx_->verifyMode));
  if (ret != 0) {
    throw TTransportException(TTransportException::NOT_OPEN, "set TLS_PEER_VERIFY failed");
  }
...
void TSSLSocket::open() {
  ...
  if (error) {
    string errStr
        = "TSocket::open() getaddrinfo() " + getSocketInfo() + string(THRIFT_GAI_STRERROR(error));
    GlobalOutput(errStr.c_str());
    close();
...
TSSLSocketFactory::TSSLSocketFactory(SSLProtocol protocol)
  ...
  case TLSv1_2:
    ...
    break;
  default:
    throw TTransportException(TTransportException::BAD_ARGS, "Specified protocol is invalid");
  }
}
...
void TSSLSocketFactory::loadCertificateFromBuffer(const char* aCertificate, const char* format) {
  ...
  if (strcmp(format, "PEM") == 0) {
    const int status
        = tls_credential_add(Thrift_TLS_SERVER_CERT_TAG, TLS_CREDENTIAL_SERVER_CERTIFICATE,
...
void TSSLSocketFactory::loadPrivateKeyFromBuffer(const char* aPrivateKey, const char* format) {
  ...
  if (strcmp(format, "PEM") == 0) {
    const int status = tls_credential_add(Thrift_TLS_PRIVATE_KEY, TLS_CREDENTIAL_PRIVATE_KEY,
