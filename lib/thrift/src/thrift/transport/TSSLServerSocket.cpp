...
void TSSLServerSocket::listen() {
  ...
  do {
    ...
    int ret
        = setsockopt(serverSocket_, SOL_TLS, TLS_SEC_TAG_LIST, sec_tag_list, sizeof(sec_tag_list));
    if (ret != 0) {
    ...
    if (trybind->ai_family == AF_INET6) {
      int zero = 0;
      if (-1
          == setsockopt(serverSocket_, IPPROTO_IPV6, IPV6_V6ONLY, cast_sockopt(&zero),
                        sizeof(zero))) {
  ...
  if (retries > retryLimit_) {
    ...
    THRIFT_SNPRINTF(errbuf, sizeof(errbuf), "TServerSocket::listen() Could not bind to port %d",
                    port_);
